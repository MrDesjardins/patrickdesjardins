...
```typescript
const logger = store => next => action => { console.log("Before", action) const result = next(action) console.log("After", store.getState()) return result } 
``` 
...
```typescript
export function asyncMethodThatFetchData(inputData) { return function (dispatch, getState) { dispatch(actionLoadingData());

return fetch(\`https://www.api.com/${inputData.id}\`) .then(json => dispatch(actionDataLoaded(inputData, json)); ) } } store.dispatch(asyncMethodThatFetchData("123")); 
``` 
