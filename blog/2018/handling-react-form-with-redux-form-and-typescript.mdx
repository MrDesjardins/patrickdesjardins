...
const formInputsValues = state.form\[FORM\_NAME\] as ReduxFormState<MyComponentWritableModel>; // The problem is that form\[x\] contains only data from the inputs which are string
