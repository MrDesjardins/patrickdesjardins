...
```typescript
public convertToHighChartType(type: MetricsChartType): string { switch (type) { case "Line": return "line"; case "AreaDiff": return "area"; default: return exhaustiveCheck(type); } }

export function exhaustiveCheck(type: never): never { throw new Error("Missing type"); } 
``` 
