...
```typescript
class MyClass{ constructor(public myOtherInjectedClass: IMyInjectedClassTypeHere){ ...} } 
``` 
...
```typescript
function mapToMockify<T extends Object>(obj: T): Mockify<T> { let newObject: Mockify<T> = {} as Mockify<T>; const properties = Object.getOwnPropertyNames(Object.getPrototypeOf(obj)); for (let i = 0; i &lt; properties.length; i++) { newObject\[properties\[i\]\] = jest.fn(); } return newObject; } 
``` 
...
```typescript
const toTest = new MyClass(mapToMockify(new MyInjectedClassTypeHere())); expect(toTest.function1).toHaveBeenCalled(); 
``` 
