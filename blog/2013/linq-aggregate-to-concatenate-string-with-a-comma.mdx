...
\[csharp\] string flatten = inputs.Aggregate((current, next) => string.Format("{0}, {1}", current, next)) \[/csharp\]
...
\[csharp\] string flatten = string.Empty; for(int i = 0 ; i < inputs.Length ; i++) { if(i!=(i.Length-1)) { flatten += str + ", "; } else { flatten += str; } } \[/csharp\]
