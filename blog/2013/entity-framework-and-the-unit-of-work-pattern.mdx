...
modelBuilder.Entity<Animal>() .HasMany(entity => entity.Enemies) .WithMany(d => d.EnemyOf) .Map(d => d.ToTable("Animals\_Enemies\_Association").MapLeftKey("AnimalId").MapRightKey("EnemyId")); } } 
...
```csharp
public class AnimalController : Controller { private IAnimalService \_service;
