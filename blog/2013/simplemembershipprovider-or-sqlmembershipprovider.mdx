...
[![sqlSimpleProviderTables](http://patrickdesjardins.com/blog/wp-content/uploads/2013/09/sqlSimpleProviderTables.png)](http://patrickdesjardins.com/blog/?attachment_id=2284)

As you can see in the image above, you can add a Language field. This require only to add a property and let Entity Framework handle the rest. \[csharp highlight="18"\] \[Table("UserProfile")\] public class UserProfile : ICurrentUser { public UserProfile() { this.UserName = "Anonymous"; this.Language = "en-US"; }
...
\[csharp\] WebSecurity.InitializeDatabaseConnection( "DefaultConnection", "UserProfile", "UserId", "UserName", autoCreateTables: true); \[/csharp\]
...
\[csharp\] protected override void Seed(DatabaseContext context) { base.Seed(context);
...
if (!((IList<string>)Roles.GetRolesForUser("123123")).Contains(Model.Roles.ADMINISTRATOR)) Roles.AddUsersToRoles(new\[\] { "123123", "qweqwe" }, new\[\] { Model.Roles.ADMINISTRATOR }); if (!((IList<string>)Roles.GetRolesForUser("qweqwe")).Contains(Model.Roles.NORMAL)) Roles.AddUsersToRoles(new\[\] { "qweqwe" }, new\[\] { Model.Roles.NORMAL }); } \[/csharp\]
