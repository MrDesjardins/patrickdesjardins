...
Here is an example with SELECT VALUE. We receive a strongly typed set of object. \[csharp\] string queryString = "SELECT VALUE q from table1.attr1 as q"; ObjectQuery<T> query = context.CreateQuery<T>(queryString); \[/csharp\]

You can also specify which field you return if you do not want every fields. \[csharp\] string queryString = "SELECT VALUE row (q.Field1 as Field1, q.Field2 as Field2) from table1.attr1 as q)"; ObjectQuery<T> query = context.CreateQuery<T>(queryString); \[/csharp\] This time, the keyword "row" is required because it's a reserved keyword by ESQL. ROW constructs an anonymous value.

And an example that return a set of rows. As you can see, we have a DbDataRecord.

\[csharp\] string queryString = "SELECT q table1.attr1 as q"; ObjectQuery<DbDataRecord> query = context.CreateQuery<DbDataRecord>(queryString); \[/csharp\]
