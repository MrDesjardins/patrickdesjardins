...
The first approach to handle this is to go fast and to not create a class that contain a list of integer and one integer. We can do it by using the dynamic keyword. \[csharp\] \[HttpPost\] public JsonResult SaveWorkoutSessionOrder() { dynamic json = JsonConvert.DeserializeObject(Request.Form.Get(0));
...
try { ServiceFactory.Workout.UpdateSessionOrderOnly(workout); } catch (DataNotFoundException) { throw new HttpException((int)HttpStatusCode.Forbidden,"Cannot update the session"); } return Json(new { Status="Saved" }); } \[/csharp\]
...
\[csharp\] \[HttpPost\] public JsonResult SaveWorkoutSessionOrder(WorkoutSessionOrder workoutSessionOrder) { int order = 1; var workout = new Workout { Id = workoutSessionOrder.WorkoutId }; workout.Sessions = new List<WorkoutSession>();
...
\[javascript hightlight="20"\] $(document).on("click", "#btnSaveOrder", function () { var button = $(this); $(button).attr('disabled', 'disabled'); $('.loadingAnimationfixed').show(); var listIdOrdered = \[\]; $('#sessiongrid tbody tr').each(function () { listIdOrdered.push($(this).attr("data-workoutsessionid")); });
