...
\[csharp\] protected void Application\_Start() { RegisterRoutes(RouteTable.Routes); }

public static void RegisterRoutes(RouteCollection routes) { routes.MapRootArea("{area}/{controller}/{action}/{id}", "RouteNameHere", new { area="area1", controller = "controller1", action = "action1", id = "" }); } \[/csharp\]
...
Step1: Inside Global.asax.cs \[csharp\] protected void Application\_Start() { AreaRegistration.RegisterAllAreas(); } \[/csharp\]

Step2: Inside every folders of every area you have create a class that inherit from AreaRegistration \[csharp\] public class MyArea\_AreaRegistration : AreaRegistration { public override string AreaName { get { return "MyArea"; } }
...
context.Routes.Add(string.Format("{0}\_Default", areaName), new Route(defautlRouteUrl, defaultRouteValueDictionary, dataTokensDictionary, new MvcRouteHandler())); } } \[/csharp\]
...
\[csharp\] context.Routes.MapRoute("DefaultRouteForAreaXYZ" , "AreaXYZ/{controller}/{action}/{id}" , new { controller="Home" , action="index" , id= UrlParameter.Optional } ); \[/csharp\]
