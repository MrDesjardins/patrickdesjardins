...
```csharp
 modelBuilder .Entity<Exercise>() .HasRequired(b => b.Muscle) .WithMany(m => m.ExerciseRelated) .IsIndependent() .Map(p=>p.MapKey(m => m.MuscleID, "Muscle\_ID") ; 
```
...
```csharp
 modelBuilder .Entity<Exercise>() .HasRequired(b => b.Muscle) .WithMany(m => m.ExerciseRelated) .Map(p=>p.MapKey("Muscle\_\_\_ID")) ; 
```
