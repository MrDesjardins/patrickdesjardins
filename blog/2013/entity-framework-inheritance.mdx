...
```csharp
 public class TestContext:DbContext { public TestContext() : base("TestContext") {
...
```csharp
 using (var context = new DataAccessLayer.TestContext()) { context.Encyclopedia.Add(new Models.Encyclopedia { ISBN = "123" }); context.SaveChanges(); } 
```
...
protected override void OnModelCreating(DbModelBuilder modelBuilder) { modelBuilder.Conventions.Remove<PluralizingTableNameConvention>(); modelBuilder.Entity<Magazine>().ToTable("Magazine"); modelBuilder.Entity<Encyclopedia>().ToTable("Encyclopedia"); modelBuilder.Entity<Book>().ToTable("Book").HasKey(d => d.ISBN); } } 
```
...
modelBuilder.Entity<Encyclopedia>().Map(m => { m.MapInheritedProperties(); m.ToTable("Encyclopedia"); }); modelBuilder.Entity<Magazine>().HasKey(d => d.ISBN); modelBuilder.Entity<Encyclopedia>().HasKey(d => d.ISBN); } } 
```
