...
(function($){ 
  $.fn.vibrate = function(options) { 
    ...
    var control = $(this); 
    var vibrateIndex = 0; 
    var selector; vibrateIndex = setInterval(vibe, options.interval); 
    setTimeout(stopVibration, options.duration); 
    function vibe() { 
      var shake = 3; 
      $(control).stop(true, false)
      .css({ position: 'relative', left: Math.round(Math.random() * shake) + 'px', top: Math.round(Math.random() * shake) +'px', borderColor: 'red' }); };
