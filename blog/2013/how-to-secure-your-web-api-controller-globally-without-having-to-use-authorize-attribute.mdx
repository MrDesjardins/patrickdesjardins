...
\[csharp highlight="4"\] public static void RegisterGlobalFilters(GlobalFilterCollection filters) { filters.Add(new HandleErrorAttribute()); filters.Add(new AuthorizeAttribute()); } \[/csharp\]

But, this won't work with the Api controller. You have to set the AuthorizeAttribute to the WebApiConfig file.

\[csharp highlight="9"\] public static void Register(HttpConfiguration config) { config.Routes.MapHttpRoute( name: "DefaultApi", routeTemplate: "api/{controller}/{id}", defaults: new { id = RouteParameter.Optional } );

config.Filters.Add(new AuthorizeAttribute()); } \[/csharp\]
