...
\[html\] @Html.Input().ActionLink("Edit", "Edit", null, (new { id=item.Id })) @Html.ActionLink("Details", "Details", new { id=item.Id }) \[/html\]

On the controller side, you will see the Edit action with the authorization attribute.

\[csharp\] \[HttpGet\] \[Authorize(Roles = Roles.ADMINISTRATOR)\] public ActionResult Edit(int id) { //... } \[/csharp\]

First of all, we need to create a class to be able to get the attribute of the action. Here is the class.

\[csharp\] public class AttributeHelper { private readonly HtmlHelper \_htmlHelper; public AttributeHelper(HtmlHelper htmlHelper) { \_htmlHelper = htmlHelper; }
