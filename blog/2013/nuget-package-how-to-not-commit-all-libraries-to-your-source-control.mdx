...
But, how can we send a project into a repository and have NuGet knows that it needs to download every references on the first build? This can be done by changing the solution to use **NuGet Package Restore**. To enable this feature, right click the solution in the Solution Explorer and select _Enable Restore NuGet Package Restore_. ![](images/nugetfiles.png)
