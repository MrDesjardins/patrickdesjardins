...
\[sql\] ALTER DATABASE YourDatabase SET ENABLE\_BROKER GO \[/sql\]

By doing this SQL statement, you may have error like this one : "error 9772: The Service Broker in database 'YourDatabase' cannot be enabled because there is already an enabled Service Broker with the same ID". If this happen, just execute : \[sql\] ALTER DATABASE YourDatabase SET ENABLE\_BROKER WITH ROLLBACK IMMEDIATE GO ALTER DATABASE YourDatabase SET NEW\_BROKER GO \[/sql\]

Next move is to enable notification with IIS.

\[sql\] GRANT SUBSCRIBE QUERY NOTIFICATIONS TO "TESTSERVER\\ASPNET" \[/sql\]
...
\[csharp\] SqlDependency.Start(ConfigurationManager.ConnectionStrings\["yourConnectionString"\].ConnectionString); \[/csharp\]

You can also make it stop.

\[csharp\] SqlDependency.Stop(ConfigurationManager.ConnectionStrings\["yourConnectionString"\].ConnectionString); \[/csharp\]
...
HttpContext.Current.Cache.Insert(tableName, result, dependency); } } \[/csharp\]
...
[![sqltablenotification](http://patrickdesjardins.com/blog/wp-content/uploads/2013/07/sqltablenotification.png)](http://patrickdesjardins.com/blog/?attachment_id=2183)
