...
```html  <input type="submit" name="action" value="Save" /> <input type="submit" name="action" value="Save and Continue" /> <!-- OR --> <button type="submit" name="action" value="save">Save</button> <button type="submit" name="action" value="saveAndContinue">Save and Continue</button> 
``` 
...
```csharp
 public ActionResult Register(string action, YourViewModel viewModel) { if (button == "save") { } //... } 
```
