...
[![IIS_Application_Initialization](http://patrickdesjardins.com/blog/wp-content/uploads/2014/01/IIS_Application_Initialization-400x212.png)](http://patrickdesjardins.com/blog/iis-8-0-application-initialization-for-fast-startup-everytime/iis_application_initialization)

With IIS 7.5 and after, it is possible to select the **Start Mode**. The Start mode can be "On Demand" or "Always Running". "On demand" starts the application pool from a HTTP request. "Always running" uses the Windows Process Activation Service (WAS) to start the application pool. This was available with IIS 7.5 but IIS 8.0 does have this configuration within IIS without having to modify any applicationhost.config file. It is more convenient. [![IIS_AppPool_AlwaysRunning](http://patrickdesjardins.com/blog/wp-content/uploads/2014/01/IIS_AppPool_AlwaysRunning-362x400.png)](http://patrickdesjardins.com/blog/iis-8-0-application-initialization-for-fast-startup-everytime/iis_apppool_alwaysrunning)
