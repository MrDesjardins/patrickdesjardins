...
```csharp
 this.RouteData.Values[Constants.RoutePortionName.ACTIVE\_CURRENT\_CONTEST\_ID] 
```
...
```csharp
 context.MapRoute(Constants.Areas.CONTEST + "\_contest\_activated" , Constants.Areas.CONTEST + "/{" + Constants.RoutePortionName.ACTIVE\_CURRENT\_CONTEST\_ID + "}/{controller}/{action}/{id}" , defaultValueObject , constraintsObject); 
```
