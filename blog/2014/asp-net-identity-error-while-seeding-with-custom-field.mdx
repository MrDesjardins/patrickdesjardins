...
[![SeedingError](http://patrickdesjardins.com/blog/wp-content/uploads/2013/12/SeedingError-400x58.png)](http://patrickdesjardins.com/blog/asp-net-identity-error-while-seeding-with-custom-field/seedingerror)

The UserId is a column that is ignored in the configuration of the entity. Why does it try to read from it?

Here is how the DbContext looks like: \[csharp\] public class Configuration : DbMigrationsConfiguration<DatabaseContext> { public Configuration() { }
...
var role2 = new IdentityUserRole { Role = new IdentityRole(Model.Roles.NORMAL) }; var user2 = new ApplicationUser() { UserName = "qweqwe", Email = "qweqwe@qweqwe.com", Language = "fr-CA" }; user.Roles.Add(role2); IdentityResult result2 = manager.Create(user2, "qweqwe"); //... } } \[/csharp\]
...
\[csharp\] var userStore = new UserStore<ApplicationUser>(context); \[/csharp\]
