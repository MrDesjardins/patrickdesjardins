...
\[csharp\] public static class MyHtmlHelpers { public static MvcHtmlString OutputName(this HtmlHelper helper, string name) { return new MvcHtmlString(string.Format("<h1>{0}</h1>",name)); } } \[/csharp\]

This can be used into views this way:

\[csharp\] @Html.OutputName("Patrick") \[/csharp\]
...
\[csharp\] @Html.Input().SelectorFor("MyPropertyName", new\[\] { "Item1","Item2"}) @Html.Image()... //And so on. \[/csharp\]

First, we need to inject the method for our namespaces.

\[csharp\] public static class HtmlHelperNameSpaces { public static HelperExtensionInputFactory Input(this HtmlHelper helper) { return new HelperExtensionInputFactory(helper); }

public static HelperExtensionInputFactory<TModel> Input<TModel>(this HtmlHelper<TModel> helper) { return new HelperExtensionInputFactory<TModel>(helper); } } \[/csharp\]
...
\[csharp\] /// <summary> /// Generic /// </summary> public class HelperExtensionInputFactory<TModel> : HelperExtensionInputFactory { private HtmlHelper<TModel> HtmlHelper { get; set; }
...
foreach (var item in items) { var optionBuilder = new TagBuilder("option"); optionBuilder.MergeAttribute("value", item); optionBuilder.SetInnerText(item); selectBuilder.InnerHtml += optionBuilder.ToString(); } return new MvcHtmlString(selectBuilder.ToString()); } } \[/csharp\]
...
[![HtmlHelper](http://patrickdesjardins.com/blog/wp-content/uploads/2014/01/HtmlHelper-400x79.png)](http://patrickdesjardins.com/blog/how-to-create-your-own-asp-net-mvc-html-helper-namespace/htmlhelper)
