...
\[csharp highlight="5"\] public class MyDbContext : DbContext { protected override void OnModelCreating(DbModelBuilder modelBuilder) { modelBuilder.Entity<YourEntity>().HasKey(d => d.Id); base.OnModelCreating(modelBuilder); } } \[/csharp\]
...
\[csharp highlight="1"\] public class YourEntityConfiguration : EntityTypeConfiguration<YourEntity> { public YourEntityConfiguration() { this.HasKey(d => d.Id); } } \[/csharp\] Once you have your classes defined, the OnModelCreating can register them.

\[csharp highlight="5"\] public class MyDbContext : DbContext { protected override void OnModelCreating(DbModelBuilder modelBuilder) { modelBuilder.Configurations.Add(new YourEntityConfiguration()); base.OnModelCreating(modelBuilder); } } \[/csharp\]
...
\[csharp\] public class MyDbContext : DbContext { protected override void OnModelCreating(DbModelBuilder modelBuilder) { modelBuilder.Configurations.AddFromAssembly(Assembly.GetAssembly(GetType())); //Current Assembly base.OnModelCreating(modelBuilder); } } \[/csharp\]
