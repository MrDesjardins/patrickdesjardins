...
\[csharp\] public string Url(string action, string controller, string area = null, object routeValues = null) { //Code here } \[/csharp\]
...
\[csharp\] Url("action", "controller", "area", new {Id = "123", Name="This is my name"}); \[/csharp\]
...
\[csharp\] public static class ObjectExtensions { public static IDictionary<string, object> AsDictionary(this object source, BindingFlags bindingAttr = BindingFlags.DeclaredOnly | BindingFlags.Public | BindingFlags.Instance) { return source.GetType().GetProperties(bindingAttr).ToDictionary ( propInfo => propInfo.Name, propInfo => propInfo.GetValue(source, null) ); } } \[/csharp\] It uses reflection to get all properties and from these properties to get all values.

Here is a unit test for the AsDictionary extension method.

\[csharp\] \[TestClass\] public class ObjectExtensionsTest { \[TestMethod\] public void GivenAnObject\_WhenThisOneHasMultipleProperties\_ThenDictionary() { // Arrange var objectToConvert = new {Id="Test", Name="Test2"};

// Act var dictionary = objectToConvert.AsDictionary();

// Assert Assert.AreEqual(2,dictionary.Keys.Count); Assert.AreEqual(objectToConvert.Id,dictionary\["Id"\]); Assert.AreEqual(objectToConvert.Name,dictionary\["Name"\]); } } \[/csharp\]
