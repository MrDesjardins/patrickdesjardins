...
Once paths are define you can now decide which JavaScript file to load for the page you want. The example below is from RequireJs. It loads JQuery, a JavaScript for canvas and something that use a predefined subpath called "app" that us a module named "sub". ```typescript
requirejs(\['jquery', 'canvas', 'app/sub'\], function ($, canvas, sub) { // Every module are loaded here }); 
``` 
...
Let's dive in a short demo. Create a simple Html file with that call the bootstrap of Require.js. \[html\] <!DOCTYPE html> <html> <head> <script data-main="scripts/main" src="scripts/require.js"></script> </head> <body> </body> </html> \[/html\]
...
[![requireExample](http://patrickdesjardins.com/blog/wp-content/uploads/2014/08/requireExample.png)](http://patrickdesjardins.com/blog/intro-how-to-load-asynchronously-javascript-files/requireexample)

The main JavaScript will require the use of the other JavaScript in the other folder. ```typescript
console.log("Main: Before Require"); require(\["helper/util"\], function(util) { console.log("Main: After Util loaded"); }); console.log("Main: After Require"); 
``` 
 And the util will be a plain JavaScript that just show in the console the output. ```typescript
...
[![requireOutput](http://patrickdesjardins.com/blog/wp-content/uploads/2014/08/requireOutput.png)](http://patrickdesjardins.com/blog/intro-how-to-load-asynchronously-javascript-files/requireoutput)
...
```typescript
console.log("Main: Before Require"); require(\["helper/util","helper/util2","helper/util3"\], function(util) { console.log("Main: After all Util loaded"); }); console.log("Main: After Require"); 
``` 
 The output is about the same but now we have three JavaScript file loaded in parallel. [![requireAsync](http://patrickdesjardins.com/blog/wp-content/uploads/2014/10/requireAsync-400x127.png)](http://patrickdesjardins.com/blog/intro-how-to-load-asynchronously-javascript-files/requireasync)

Let's do a comparison of these JavaScript files loaded without Require.js to see how it would have looks. [![requireNoRequireOuputPerformance](http://patrickdesjardins.com/blog/wp-content/uploads/2014/10/requireNoRequireOuputPerformance-400x119.png)](http://patrickdesjardins.com/blog/intro-how-to-load-asynchronously-javascript-files/requirenorequireouputperformance)
