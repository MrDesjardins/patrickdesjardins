...
\[csharp\] using (var context = new YourContext()) { // Create a new instance with Add var person = new Person {Name = "New person with New Keyword",BirthDate = DateTime.Now}; Console.WriteLine("Person non-proxy state: " + context.Entry(person).State); context.Persons.Add(person);// Now Entity Framework added the object but was not tracking anything
...
[![ProxyNonProxyCreate](http://patrickdesjardins.com/blog/wp-content/uploads/2014/07/ProxyNonProxyCreate.png)](http://patrickdesjardins.com/blog/entity-framework-create-entity/proxynonproxycreate)
