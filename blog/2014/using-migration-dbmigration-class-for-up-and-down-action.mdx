...
Before talking about commands that you will use during the development. One command is required to be executed once. It is the one that configure the migration tool. \[csharp\] Enable-Migrations \[/csharp\]
...
\[code\] param(\[String\]$migrationName)

Write-Host "MigrationName: $migrationName"

Add-Migration -Name $migrationName \` -ConfigurationType YourDLL.Migration.YourDBContextConfiguration \` -StartUpProjectName YourStartupDLL \` -ProjectName YourDLL.Migration \` -Verbose \[/code\] Usage: \[code\] .\\AddMigration.ps1 TheMigrationClassToBeGenerated \[/code\]
...
\[code\] Update-Database -ConfigurationTypeName "YourDLL.Migration.YourDBContextConfiguration" \` -ProjectName "YourDLL.Migration" \` -StartUpProject "YourStartupDLL" \` -Verbose \[/code\]
