...
\[xml highlight="7,8"\] <appSettings> <add key="webpages:Version" value="2.0.0.0" /> <add key="webpages:Enabled" value="false" /> <add key="PreserveLoginUrl" value="true" /> <add key="ClientValidationEnabled" value="true" /> <add key="UnobtrusiveJavaScriptEnabled" value="true" />

<add key="RecaptchaPrivateKey" value="123nI9cSAAAAALtUefffnLRn2pWb7IpNrtOGQjzz" /> <add key="RecaptchaPublicKey" value="123nI9cSAAAAAOH3eee3t1ZaZ6rRUwHQK4KMimfu" /> </appSettings> \[/xml\]
...
\[csharp\] \[RecaptchaControlMvc.CaptchaValidator\] public ActionResult Register(RegisterModel model, bool captchaValid, string captchaErrorMessage) { if (ModelState.IsValid) { if (!captchaValid) { ModelState.AddModelError("recaptcha", captchaErrorMessage); return View(model); } //... \[/csharp\] You need first to add the attribute that will call the server and do the validation. Second, you need to add 2 variables to the action that will tell you if the captcha is valid and the message. You can then check the value and display the form again if it's wrong.
