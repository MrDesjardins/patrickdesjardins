...
The first step is to create your method into your DAL. \[csharp\] public IQueryable<MyModel> GetMyModel() { //Linq to Entity is returned here } \[/csharp\]
...
\[csharp\] public IQueryable<MyModel> GetMyModel() { var query1 = //Linq to Entity with condition for > 100$ var query2 = //Linq to Entity with condition for > 20$ && Discount > 0$ //return query1 + query 2 } \[/csharp\]
...
\[csharp\] var set1 = new int\[\] { 1, 2, 3 }; var set2 = new int\[\] { 3, 4, 5 };

var result= set1.AsQueryable().Concat(set2.AsQueryable()); // count == 6 var result2 = set1.AsQueryable().Union(set2.AsQueryable()); // count == 5 \[/csharp\]

Our example could look with union like this: \[csharp\] public IQueryable<MyModel> GetMyModel() { var query1 = //Linq to Entity with condition for > 100$ var query2 = //Linq to Entity with condition for > 20$ && Discount > 0$ return query1.union(query2); //Both condition } \[/csharp\]
