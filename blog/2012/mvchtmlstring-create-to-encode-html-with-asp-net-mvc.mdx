...
\[csharp\] public static class HtmlHelpers { public static MvcHtmlString HtmlEncode(this string data) { return MvcHtmlString.Create(data); } } \[/csharp\]

This extension will let you encode any string so you could use it with you properties.

\[csharp\] This is my property : @Model.Name.HtmlEncode() \[/csharp\]
...
\[csharp\] public IHtmlString Raw(string value) { return new HtmlString(value); }

public IHtmlString Raw(object value) { return new HtmlString(value == null ? null : value.ToString()); } \[/csharp\]

This is different from MvcHtmlString.Create()

\[csharp\] public sealed class MvcHtmlString : HtmlString { public static readonly MvcHtmlString Empty = Create(String.Empty);
...
public static bool IsNullOrEmpty(MvcHtmlString value) { return (value == null || value.\_value.Length == 0); } } \[/csharp\]

Are you can see, this one return a MvcHtmlString. At the end, both return something similar and encoded html.

In the scenario you want to display the html's string you simply need to use the variable: \[csharp highlight="5"\] @{ var x= "<b>Test</b>"; }
