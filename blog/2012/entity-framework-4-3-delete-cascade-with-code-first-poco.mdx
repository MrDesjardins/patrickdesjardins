...
```sql
 CREATE TABLE [Parent]( [ID] [int] IDENTITY(1,1) NOT NULL, [Description] [nvarchar](100) NULL, CONSTRAINT [PK\_Parent] PRIMARY KEY CLUSTERED GO
...
ALTER TABLE [ParentDetail] WITH CHECK ADD CONSTRAINT [FK\_ParentDetail\_Parent] FOREIGN KEY([Parent\_ID]) REFERENCES [Parent] ([ID]) ON DELETE CASCADE GO 
```
