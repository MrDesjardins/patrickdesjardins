...
```csharp
 public interface IDatabaseContext { int SaveChanges(); IDbSet<TEntity> SetOwnable<TEntity>() where TEntity : class, IUserOwnable; DbSet<TEntity> Set<TEntity>() where TEntity : class; DbEntityEntry<TEntity> Entry<TEntity>(TEntity entity) where TEntity : class; void InitializeDatabase(); UserProfileImpersonate Impersonate(ICurrentUser userProfile); } 
```
...
```csharp
 public class DatabaseContext : DbContext, IDatabaseContext { public const string DEFAULTCONNECTION = "DefaultConnection";
...
```csharp
 using (var db = DatabaseContext.Impersonate(new UserProfile { UserId = 1 })) { db.SetOwnable<Workout>().Add(entity); } 
...
```csharp
 public class UserProfileImpersonate : IDatabaseContext, IDisposable { private readonly DatabaseContext \_databaseContext;
