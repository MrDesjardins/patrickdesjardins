...
\[csharp\] void myCanvas\_MouseRightButtonDown(object sender, MouseButtonEventArgs e) { e.Handled = true; Point currentMousePosition = e.GetPosition(this.myCanvas); var underMouseControls = GetControlFromCoordonate(currentMousePosition); } \[/csharp\]

The second step is to create the method _**GetControlFromCoordonate**_ to return a list of Control that has been clicked.

\[csharp\] public IEnumerable<Control> GetControlFromCoordonate (Point coordonate) { return VisualTreeHelper.FindElementsInHostCoordinates(coordonate, this.myCanvas).OfType<Control>(); } \[/csharp\]
