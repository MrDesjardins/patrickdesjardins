...
\[csharp\] public ActionResult Edit(Car car){... \[/csharp\]
...
\[csharp\] \[HttpPost\] public ActionResult Edit(Car car) { if(ModelState.IsValid) { //Update code to be placed here

return RedirectToAction("CarList"); } else { return View("CarEdit",car); } } \[/csharp\]
...
\[csharp highlight="21,20"\] public class Car: IValidatableObject { public int Id { get; set; } public string Name { get; set; } public string Type { get; set; } public string Color { get; set; }

public IEnumerable<ValidationResult> Validate(ValidationContext validationContext) { if(string.IsNullOrEmpty(Name)) { yield return new ValidationResult("Name is mandatory", new\[\] {"Name"}); } if (string.IsNullOrEmpty(Type)) { yield return new ValidationResult("Type is mandatory", new\[\] { "Type" }); } if (string.IsNullOrEmpty(Color)) { yield return new ValidationResult("\*", new\[\] { "Color" }); yield return new ValidationResult("Color is mandatory"}); } } } \[/csharp\]
...
\[csharp highlight="6"\] \[HttpPost\] public ActionResult Edit(Car car) { if(IsCarAvailable()) { ModelState.AddModelError(string.Empty,"Car cannot be edited because not available anymore"); }

if(ModelState.IsValid) { //Update code to be placed here
...
\[csharp\] var allErrors = ModelState.Values.SelectMany(e => e.Errors).Select(gh => gh.ErrorMessage); \[/csharp\]

[![](http://patrickdesjardins.com/blog/wp-content/uploads/2012/04/allerrorsInOneCollection-400x51.png "allerrorsInOneCollection")](http://patrickdesjardins.com/blog/how-to-validate-model-object-with-asp-net-mvc-correctly/allerrorsinonecollection)
