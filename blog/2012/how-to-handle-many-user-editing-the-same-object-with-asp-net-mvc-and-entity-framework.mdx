...
\[csharp highlight="5,6"\] public class BaseEntity { public int Id { get; set; }

\[ConcurrencyCheck\] \[Timestamp\] public Byte\[\] Timestamp { get; set; } } \[/csharp\]
...
\[csharp\] @Html.HiddenFor(model => model.Timestamp) \[/csharp\]

So, the whole view look like this: \[csharp\] @using (Html.BeginForm()) { @Html.HiddenFor(model => model.Timestamp) @Html.ValidationSummary(true) <fieldset> <legend>Customer</legend>
...
<p> <input type="submit" value="Save" /> </p> </fieldset> } \[/csharp\]

The model look like:

\[csharp\] public class Customer:BaseEntity { public string FirstName { get; set; } public string LastName { get; set; } } public class BaseEntity { public int Id { get; set; } \[ConcurrencyCheck\] \[Timestamp\] public Byte\[\] Timestamp { get; set; } } \[/csharp\]

And the controller which contain the compare code: \[csharp\] \[HttpPost\] public ActionResult Edit(Customer customer) { if (ModelState.IsValid) { db.Entry(customer).State = EntityState.Modified; try { db.SaveChanges(); } catch (DbUpdateConcurrencyException ex) { var entry = ex.Entries.Single(); var databaseValues = (Customer)entry.GetDatabaseValues().ToObject(); var clientValues = (Customer)entry.Entity; if (databaseValues.FirstName != clientValues.FirstName) ModelState.AddModelError("Name", "Current value: " + databaseValues.FirstName); if (databaseValues.LastName != clientValues.LastName) ModelState.AddModelError("Budget", "Current value: " + databaseValues.LastName);
...
[![](http://patrickdesjardins.com/blog/wp-content/uploads/2012/09/concurrencyException-400x382.png "concurrencyException")](http://patrickdesjardins.com/blog/how-to-handle-many-user-editing-the-same-object-with-asp-net-mvc-and-entity-framework/concurrencyexception)

If we go check in the database, we will see that the table contains a hexadecimal value inside the TimeStamp.

[![](http://patrickdesjardins.com/blog/wp-content/uploads/2012/09/timstampdatabase.png "timstampdatabase")](http://patrickdesjardins.com/blog/how-to-handle-many-user-editing-the-same-object-with-asp-net-mvc-and-entity-framework/timstampdatabase)
...
\[csharp\] public int Update(Muscle entity) { Muscle fromDatabase = Get(entity.Id); DatabaseContext.Entry(fromDatabase).CurrentValues.SetValues(entity); DatabaseContext.Entry(fromDatabase).State = EntityState.Modified; } \[/csharp\] By this: \[csharp\] public int Update(Muscle entity) { this.Set<Muscle>().Attach(entity); this.ChangeTracker.Entries<Muscle>().Single(d => d.Entity == entity).State = EntityState.Modified; } \[/csharp\]
