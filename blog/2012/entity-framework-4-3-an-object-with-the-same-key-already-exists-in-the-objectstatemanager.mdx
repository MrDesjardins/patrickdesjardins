...
\[csharp\] if (!Database.Set<MyObject>().Local.Any(e => e.ID == myObject.ID)) Database.MyObjects.Attach(myObject); else myObject = Database.Set<MyObject>().Local.Single(e => e.ID == myObject.ID); \[/csharp\]
