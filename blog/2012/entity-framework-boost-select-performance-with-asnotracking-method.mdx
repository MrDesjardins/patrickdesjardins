...
\[csharp\] public class Customer:BaseEntity { public string FirstName { get; set; } public string LastName { get; set; } } \[/csharp\]
...
\[sql\] DECLARE @count INT DECLARE @firstname varchar(100) DECLARE @lastname varchar(100)
...
SET @count = (@count + 1) END \[/sql\]

Finally, we will time the time it takes to Entity Framework to generate the loading of the object into the view.

\[csharp highlight="4,6"\] public ViewResult Index() { var watch = new Stopwatch(); watch.Start(); var customers = db.Customers.Include(c => c.License).ToList(); watch.Stop(); ViewBag.TimeElapsed = watch.ElapsedMilliseconds; return View(customers); } \[/csharp\]

The result from several test give me (on my machine) **237ms**. Now, let's do the same test with AsNoTracking().

\[csharp highlight="5"\] public ViewResult Index() { var watch = new Stopwatch(); watch.Start(); var customers = db.Customers.Include(c => c.License).AsNoTracking().ToList(); watch.Stop(); ViewBag.TimeElapsed = watch.ElapsedMilliseconds; return View(customers); } \[/csharp\]
...
\[csharp\] public virtual IInternalQuery<TElement> AsNoTracking() { return (IInternalQuery<TElement>) new InternalQuery<TElement>(this.\_internalContext, (ObjectQuery) DbHelpers.CreateNoTrackingQuery((ObjectQuery) this.\_objectQuery)); } \[/csharp\]
...
\[csharp highlight="5"\] public static IQueryable CreateNoTrackingQuery(ObjectQuery query) { IQueryable queryable = (IQueryable) query; ObjectQuery objectQuery = (ObjectQuery) queryable.Provider.CreateQuery(queryable.Expression); objectQuery.MergeOption = MergeOption.NoTracking; return (IQueryable) objectQuery; } \[/csharp\]
...
\[csharp\] ObjectContext objectContext = ((IObjectContextAdapter)dbContext).ObjectContext; \[/csharp\]
