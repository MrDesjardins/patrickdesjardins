...
\[csharp\] if (lastMethod = currentOneInvoked) { Invoke(currentOneInvoked); } else if(listMethod.Find(currentOneInvoked)) { Invoke(currentOneInvoked); } else { UpdateAndInvoke(currentOneInvoked, arg0, ...); } \[/csharp\]
