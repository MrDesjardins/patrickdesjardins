...
```csharp
 var sessionCookie = new HttpCookie("ASP.NET\_SessionId", string.Empty); sessionCookie.Expires = DateTime.Now.AddDays(-1); sessionCookie.Domain = UrlInformation.GetHostDomain(HttpContext.Current.Request.Url.Host); HttpContext.Current.Response.Cookies.Add(sessionCookie); 
```
