...
\[csharp\] \[AcceptVerbs(HttpVerbs.Post)\] public ActionResult Edit(FormCollection form) { // Get movie to update var id = Int32.Parse(form\["id"\]); var movieToUpdate = \_db.MovieSet.First(m => m.Id == id);

// Deserialize (Include white list!) TryUpdateModel(movieToUpdate, new string\[\] { "Title", "Director" }, form.ToValueProvider());

// If valid, save movie to database if (ModelState.IsValid) { \_db.SaveChanges(); //... } \[/csharp\]

Or, you could use the ability of auto binding each http parameters:

\[csharp\] \[HttpPost\] public ActionResult Edit(Course course) { if (ModelState.IsValid) { db.Entry(course).State = EntityState.Modified; db.SaveChanges(); //... } \[/csharp\]
...
\[csharp\] public void Update(MyObject obj) { //Update all but not relationship (scalar properties are updated) var fromDatabase = Database.Customers.Where(p=>p.Id==customer.ID); Database.Entry(fromDatabase).CurrentValues.SetValues(obj);
...
return Database.SaveChanges(); } \[/csharp\]
