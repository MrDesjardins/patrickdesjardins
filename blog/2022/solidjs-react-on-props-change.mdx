...
So, there is a caveat here, what if we want to change the background without using `console.log`? Removing the `console.log` makes the component renders once, as we saw earlier. So, at the moment, the best I found was to use the variable without assigning.
