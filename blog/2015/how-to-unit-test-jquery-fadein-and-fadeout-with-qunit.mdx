...
\[javascript highlight="6"\] test("should animate element over 500ms", function () { var el = jQuery("<div></div>"); el.appendTo(document.body); ok(el.css('display').indexOf("block") > -1); el.fadeOut(); this.clock.tick(510); ok(el.css('display').indexOf("none") > -1); }); 
``` 


[![testpass](http://patrickdesjardins.com/blog/wp-content/uploads/2015/05/testpass-400x97.png)](http://patrickdesjardins.com/blog/?attachment_id=4215)
