...
public string GetDomainNameWithPort() { var stringDomain = HttpContext.Current.Request.Url.Authority; return stringDomain; } } \[/csharp\]

Having these interface allows to easily mock those interfaces. You can have in your code initialization something that setup your running context. Here is a code with **Moq Mocking Framework**. \[csharp\] protected Mock<IRunningContext> runningContext; //... \[TestInitialize\] public void InitializeBetweenTest() { runningContext = new Mock<IRunningContext>(); runningContext.Setup(d => d.GetCurrentTime()).Returns(new DateTime(2014, 03, 02)); runningContext.Setup(d => d.GetUserId()).Returns(ApplicationUser.SYSTEM\_TEST\_USER\_ID); } \[/csharp\]
