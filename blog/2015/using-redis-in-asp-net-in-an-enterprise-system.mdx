...
```csharp
 public Contest GetById(int id) { var key = string.Format("contest\_by\_id\_", id); var contestObject = this.cache.Get<Contest>(key); if (contestObject == null) { contestObject = this.contestRepository.GetById(id); this.cache.Set(key, contestObject); } return contestObject; } 
```
