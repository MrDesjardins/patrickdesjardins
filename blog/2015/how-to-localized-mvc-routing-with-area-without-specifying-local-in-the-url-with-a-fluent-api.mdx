...
```csharp
 routes.MapRoute(Constants.Areas.CONTEST + "\_contest\_detail" , Constants.Areas.CONTEST + "/{" + Constants.RoutePortionName.ACTIVE\_CURRENT\_CONTEST\_ID + "}/Detail" , new RouteValueDictionary { { Constants.RoutePortionName.ACTION, "Detail" }, { Constants.RoutePortionName.CONTROLLER, "UserContest" }} , new RouteValueDictionary {{ Constants.RoutePortionName.ACTIVE\_CURRENT\_CONTEST\_ID,@"\\d+"} }); 
```
