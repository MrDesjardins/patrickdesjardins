...
\[csharp\] public class House { public int Id { get; set; } public double Price { get; set; } public Address Address { get; set; } }
...
\[csharp\] public class MyEntity { public MyComplexClass Property1 { get; set; } }

var entityLocal = this.Set<MyEntity>().Local.SingleOrDefault(d => d.Id == entity.Id); if (entityLocal == null) { entityLocal = this.Set<MyEntity>().Attach(entity); } this.ChangeTracker.Entries<MyEntity>().Single(d => d.Entity == entityLocal).State = EntityState.Modified; this.Entry(entity).Property(s => s.Property1.SubProperty1).IsModified = true; this.Entry(entity).Property(s => s.Property1.SubProperty2).IsModified = false;//This remove all modification of the object!!! this.SaveChanges(); \[/csharp\]

As you can see, the entity is marked as modified and I set one of the complex type property to false. The result is that none of the complex type properties will be saved. Here is the SQL generated: \[sql\] update \[dbo\].\[MyEntity\] set @p = 0 where ((\[Id\] = @0)) \[/sql\] The Sql update just the property of the entity, not the one of the complex type. However, if we do not mark the _SubProperty2_ with _IsModified_ to False, we have a SQL that save the whole entity, with the complex type. \[sql\] update \[dbo\].\[MyEntity\] set \[Property1\_SubProperty1\] = @0 , \[Property1\_SubProperty2\] = null where ((\[Id\] = @1)) \[/sql\]

I took the shortcut to profile the Sql Database to see the output but you could just check from the database context the properties changed and realize the same behavior. \[csharp\] var entry = DatabaseContext.Entry(entity); var namesOfChangedProperties = entry.CurrentValues.PropertyNames .Where(p => entry.Property(p).IsModified) .ToArray(); \[/csharp\]
