...
\[csharp\] public class ShoppingBasket { public List<Item> Items { get; set; }
...
public bool IsDiscontinued { get; set; } } \[/csharp\]
...
// Assert Assert.AreEqual(2, itemThatWeCanPurchase.Count); } \[/csharp\]
...
\[csharp\] public class Item { public int Id { get; set; }

public double Money { get; set; }

public bool IsDiscontinued { get; set; }

public bool IsPriceLegitForPurchase() { return this.Money < 0 || this.Money > 100; } } \[/csharp\]
...
\[csharp\] foreach (var item in Items) { bool canBuy = true; if (!item.IsPriceLegitForPurchase()) { canBuy = false; }

if (item.IsDiscontinued) { canBuy = false; } // ... \[/csharp\]

Instead of calling twice the item we should think. These two operations has something in common: it validates the item for purchase. We should have a method in the item that does that for us. Not only it will self-document what it is going on but that logic could be reused later without having to duplicate both calls. We end up having a single validation method for purchase in item. \[csharp\] public bool IsValidForPurchase() { if (!this.IsPriceLegitForPurchase()) { return false; }

if (this.IsDiscontinued) { return false; } return true; } \[/csharp\] The test is also clearer. \[csharp\] //... if(!item.IsValidForPurchase()) { canBuy = false; } //... \[/csharp\]
...
\[csharp\] public class ItemValidToPurchase : List<Item> { public bool HasReachMaximumAmountPerOrder() { return this.Sum(d => d.Money) > 50; } } \[/csharp\]

The condition in the Shopping Basket is just to call the method.

\[csharp\] if (canBeBought.HasReachMaximumAmountPerOrder()) { canBuy = false; } \[/csharp\]

So the final code looks like this for the test **about unique**. \[csharp\]
...
// Assert Assert.IsTrue(isUnique); } \[/csharp\]

And the classes code look. \[csharp\] public class Item { public int Id { get; set; }
...
public bool IsShoppingBasketFull() { return this.Items.Count >= 5; } } \[/csharp\]
