...
```csharp
 const string PATTERN\_WITHOUT\_NAME = @"(parser)\\((.\*?)\\)"; //This has 3 groups const string PATTERN\_WITH\_NAME= @"(parser)\\((?<parameterName>.\*?)\\)"; //This also has 3 groups 
```
...
```csharp
 var regex = new Regex(PATTERN\_WITH\_NAME, RegexOptions.IgnoreCase);
