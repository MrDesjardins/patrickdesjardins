...
Console.WriteLine("Wait All"); var task = allStrings.ToList(); var allAwaitedTask = await Task.WhenAll(task); foreach (var s in allAwaitedTask) { Console.WriteLine("->" + s); } Console.WriteLine("Finish"); 
``` [![AsyncExample2](http://patrickdesjardins.com/blog/wp-content/uploads/2014/11/AsyncExample2-400x228.png)](http://patrickdesjardins.com/blog/c-async-example/asyncexample2) This second example shows that the first two tasks end before even adding the third one.
...
Console.WriteLine("Wait All"); var task = allStrings.ToList(); var allAwaitedTask = await Task.WhenAll(task); foreach (var s in allAwaitedTask) { Console.WriteLine("->" + s); } Console.WriteLine("Finish"); 
``` To make the forth task use the result of the third task, we need to **await** the third task before the call of the forth's one.[![AsyncExample3](http://patrickdesjardins.com/blog/wp-content/uploads/2014/11/AsyncExample3-400x206.png)](http://patrickdesjardins.com/blog/c-async-example/asyncexample3) As you can see in the output, the first three tasks ends before the forth one start. In fact, only the third one is awaited but since the first two have a lower delay than the third one, these two also finish.
