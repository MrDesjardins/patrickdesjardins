...
\[sql\] declare @dob1 as decimal(16,4) set @dob1 = cast(554656545465486786844864613 as decimal(16,4)) select @dob1

declare @dob2 as decimal(16,4) set @dob2 = CONVERT(decimal(16,4),5455531234268.68423224224244864613 ) select @dob2 \[/sql\]
...
\[sql\] declare @dob3 as int set @dob3 = cast(0\*1/2 as int) select @dob3 \[/sql\]

This one was not:

\[sql\] declare @fromValue as int declare @toValue as int

set @fromValue = 1 set @toValue = 2

SELECT cast(Quantity \* @fromValue/@toValue as int) as newQuantity FROM \[Trading\].\[Stock\] \[/sql\]
...
SELECT cast(Quantity \* @fromValue/@toValue as int) as newQuantity FROM \[Trading\].\[Stock\] WHERE Quantity > 0 AND Quantity < 999999999999 \[/sql\]
