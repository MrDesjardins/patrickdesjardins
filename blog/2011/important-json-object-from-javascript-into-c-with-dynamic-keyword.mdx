...
\[csharp\] string response = HtmlPage.Window.Invoke("GetMyJson").ToString(); MyObject data = (MyObject)JsonConvert.DeserializeObject(response); \[/csharp\]
...
\[csharp\] string response = HtmlPage.Window.Invoke("GetMyJson").ToString(); dynamic data = JsonConvert.DeserializeObject(response); string s1 = data.Abc; string s2 = data.Def; \[/csharp\]
