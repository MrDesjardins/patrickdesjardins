...
\[caption id="attachment\_38" align="aligncenter" width="257" caption="Northwind's Customers and Orders schema"\][![Northwind's Customers and Orders schema](http://patrickdesjardins.com/blog/wp-content/uploads/2011/08/customersandordersNorthwind-257x300.png "Northwind's Customers and Orders")](http://patrickdesjardins.com/blog/wp-content/uploads/2011/08/customersandordersNorthwind.png)\[/caption\]
...
\[csharp\]</pre> <h1>Group EntityFramework</h1> <pre> \[/csharp\]
...
\[caption id="attachment\_44" align="aligncenter" width="139" caption="Linq Join without On"\][![Join Statement](http://patrickdesjardins.com/blog/wp-content/uploads/2011/08/join-139x300.png "Join")](http://patrickdesjardins.com/blog/wp-content/uploads/2011/08/join.png)\[/caption\]
...
\[csharp highlight="1-5"\] NorthwindEntities db = new NorthwindEntities(); var source = from c in db.Customers where c.CustomerID.StartsWith("A") join o in db.Orders on c.CustomerID equals o.CustomerID into g select new { Customer = c, Order = g }; string html = string.Empty; foreach(var cust in source) {
...
\[csharp\] var source = db.Customers.Where(c => c.CustomerID.StartsWith("A")) .GroupJoin(db.Orders, c => c.CustomerID, o => o.CustomerID, (c, o) => new { Customer = c, Order = o }); \[/csharp\] \[csharp\] var source2 = from c in db.Customers where c.CustomerID.StartsWith("A") join o in db.Orders on c.CustomerID equals o.CustomerID into g select new { Customer = c, Order = g }; \[/csharp\]

Both generate the exact same MSIL:

\[csharp\] IQueryable arg\_244\_0 = db.Customers; ParameterExpression parameterExpression = Expression.Parameter(typeof(Customers), "c"); IQueryable arg\_381\_0 = arg\_244\_0.Where(Expression.Lambda>(Expression.Call(Expression.Property(parameterExpression, (MethodInfo)MethodBase.GetMethodFromHandle(ldtoken(get\_CustomerID()))), (MethodInfo)MethodBase.GetMethodFromHandle(ldtoken(StartsWith())), new Expression\[\] { Expression.Constant("A", typeof(string)) }), new ParameterExpression\[\] { parameterExpression })); IEnumerable arg\_381\_1 = db.Orders; parameterExpression = Expression.Parameter(typeof(Customers), "c"); Expression> arg\_381\_2 = Expression.Lambda>(Expression.Property(parameterExpression, (MethodInfo)MethodBase.GetMethodFromHandle(ldtoken(get\_CustomerID()))), new ParameterExpression\[\] { parameterExpression }); parameterExpression = Expression.Parameter(typeof(Orders), "o"); Expression> arg\_381\_3 = Expression.Lambda>(Expression.Property(parameterExpression, (MethodInfo)MethodBase.GetMethodFromHandle(ldtoken(get\_CustomerID()))), new ParameterExpression\[\] { parameterExpression }); parameterExpression = Expression.Parameter(typeof(Customers), "c"); ParameterExpression parameterExpression2 = Expression.Parameter(typeof(IEnumerable), "o"); var source = arg\_381\_0.GroupJoin(arg\_381\_1, arg\_381\_2, arg\_381\_3, Expression.Lambda(Expression.New((ConstructorInfo)MethodBase.GetMethodFromHandle(ldtoken(.ctor()), typeof(<>f\_\_AnonymousType0>).TypeHandle), new Expression\[\] { parameterExpression, parameterExpression2 }, new MethodInfo\[\] { (MethodInfo)MethodBase.GetMethodFromHandle(ldtoken(get\_Customer()), typeof(<>f\_\_AnonymousType0>).TypeHandle), (MethodInfo)MethodBase.GetMethodFromHandle(ldtoken(get\_Order()), typeof(<>f\_\_AnonymousType0>).TypeHandle) }), new ParameterExpression\[\] { parameterExpression, parameterExpression2 })); \[/csharp\]

The file compare result show exact same code:

\[caption id="attachment\_45" align="aligncenter" width="300" caption="MSIL are exactly the same for GroupJoin or Join"\][![MSIL are exactly the same for GroupJoin or Join](http://patrickdesjardins.com/blog/wp-content/uploads/2011/08/filematch-300x164.png "MSIL are exactly the same for GroupJoin or Join")](http://patrickdesjardins.com/blog/wp-content/uploads/2011/08/filematch.png)\[/caption\]
