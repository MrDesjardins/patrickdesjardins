...
\[csharp\] public class Customer { public string Id { get; set; } public string Name { get; set; } public bool IsNew { get; set; } }
...
\[caption id="attachment\_349" align="aligncenter" width="388" caption="Sql Server Transaction's type"\][![](http://patrickdesjardins.com/blog/wp-content/uploads/2011/09/typetransctionsql.png "Sql Server Transaction's type")](http://patrickdesjardins.com/blog/c-transaction-for-sql-query/typetransctionsql)\[/caption\]

- ReadUncommitted
This is a very weak transaction and should not be used. In fact, this type doesn't lock anything in the database. It's possible that the data changes or be deleted by the time the commit is done. This can lead to unpredictable reaction. This type of isolation will produce for a Select statement this query: \[sql\] SELECT \* FROM CUSTOMERS WITH (NOLOCK) \[/sql\]- ReadCommitted
...
\[caption id="attachment\_350" align="aligncenter" width="382" caption="Distributed Transaction Coordinator (DTC)"\][![](http://patrickdesjardins.com/blog/wp-content/uploads/2011/09/dtc.png "dtc")](http://patrickdesjardins.com/blog/c-transaction-for-sql-query/dtc)\[/caption\]
