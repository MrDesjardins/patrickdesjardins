...
This is interesting if we compare the generated Sql from the Entity Framework and from the [Linq to Sql](./how-to-log-with-linq-to-sql "How to log with Linq to Sql?"). The  main difference is that Linq to SQL will use parameters substitution instead of directly add the value to the query. You can see it in the Where clause of the Sql query. The Linq to Sql code looks like:
