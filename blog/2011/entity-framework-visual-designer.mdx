...
[csharp line="2"] var db = new AdventureWorks\_DataEntities(); var listOfDepartment = db.DepartmentList.Where(dept => dept.DepartmentID <= 10); foreach (var d in listOfDepartment) { Console.WriteLine(string.Format("{0}:{1}",d.DepartmentID,d.Name)); } 
```
...
[csharp line="2"]
```
``` var db = new AdventureWorks\_DataEntities(); IQueryable listOfDepartement = db.DepartmentList.Where(dept => dept.DepartmentID <= 10); foreach (var d in listOfDepartement) { Console.WriteLine(string.Format("{0}:{1}",d.DepartmentID,d.Name)); } 
```
