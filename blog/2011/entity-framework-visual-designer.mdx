...
\[caption id="attachment\_77" align="aligncenter" width="400" caption="Entity Framework Data Model Wizard Dialog, step 1"\][![Entity Framework Data Model Wizard Dialog](http://patrickdesjardins.com/blog/wp-content/uploads/2011/08/newEntityFrameworkDataModel-400x276.png "newEntityFrameworkDataModel")](http://patrickdesjardins.com/blog/entity-framework-visual-designer/newentityframeworkdatamodel)\[/caption\]

From there, the second wizard step will be to select what the model will contain. We can start from a blank model or from a database. Because we want to see some functionality of the Visual Designer, let select to load the Entity Model from an existing database. For this example, I will select the [Microsoft's AdventureWorks 2008R2 SR1 database](http://msftdbprodsamples.codeplex.com "AdventureWorks Database Sql Code to create tables").

\[caption id="attachment\_78" align="aligncenter" width="400" caption="Entity Framework Model Step 2"\][![Entity Framework Model Content Wizard Step](http://patrickdesjardins.com/blog/wp-content/uploads/2011/08/efmodelcontentwizard-400x356.png "Entity Framework Model Content Wizard Step")](http://patrickdesjardins.com/blog/entity-framework-visual-designer/efmodelcontentwizard)\[/caption\]
...
\[caption id="attachment\_101" align="aligncenter" width="400" caption="Visual Designer showing Entities"\][![entityframeworkVisualDesigner](http://patrickdesjardins.com/blog/wp-content/uploads/2011/09/entityframeworkVisualDesigner-400x296.png "entityframeworkVisualDesigner")](http://patrickdesjardins.com/blog/entity-framework-visual-designer/entityframeworkvisualdesigner)\[/caption\]
...
\[caption id="attachment\_102" align="aligncenter" width="400" caption="Department's properties"\][![Entity Properties](http://patrickdesjardins.com/blog/wp-content/uploads/2011/09/entityProperties-400x155.png "entityProperties")](http://patrickdesjardins.com/blog/entity-framework-visual-designer/entityproperties)\[/caption\]
...
\[caption id="attachment\_103" align="aligncenter" width="400" caption="All department with an unique identifier under 10."\][![Department List Example](http://patrickdesjardins.com/blog/wp-content/uploads/2011/09/departementList-400x176.png "departementList")](http://patrickdesjardins.com/blog/entity-framework-visual-designer/departementlist)\[/caption\]
...
\[csharp line="2"\] var db = new AdventureWorks\_DataEntities(); var listOfDepartment = db.DepartmentList.Where(dept => dept.DepartmentID <= 10); foreach (var d in listOfDepartment) { Console.WriteLine(string.Format("{0}:{1}",d.DepartmentID,d.Name)); } \[/csharp\]

The line #2 could have been:

\[csharp line="2"\]\[/csharp\]\[/csharp\] var db = new AdventureWorks\_DataEntities(); IQueryable listOfDepartement = db.DepartmentList.Where(dept => dept.DepartmentID <= 10); foreach (var d in listOfDepartement) { Console.WriteLine(string.Format("{0}:{1}",d.DepartmentID,d.Name)); } \[/csharp\]
