...
```javascript
paths.allTypeScript = paths.typescript_in + "**/*.{ts,tsx}"; // ... gulp.task("build", () => { var compilationResults = gulp.src(paths.allTypeScript) .pipe(sourcemaps.init()) .pipe(tsProject()) compilationResults.dts.pipe(gulp.dest(paths.typescript_out)); return compilationResults.js .pipe(sourcemaps.write('.')) .pipe(gulp.dest(paths.typescript_out)); }); 
``` 
