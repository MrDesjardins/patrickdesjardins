...
```typescript
function swap(array, left, right) { var temp = array\[left\]; array\[left\] = array\[right\]; array\[right\] = temp; } function bubbleSort(arrayToSort){ var length = arrayToSort.length; for (var i = 0; i < length-1; i++){ for (var j = 0; j < length-i-1; j++){ if (arrayToSort\[j\] > arrayToSort\[j+1\]){ swap(arrayToSort, j, j+1); } } } } 
``` 
...
\[javascript highlight="3,7,11-13"\] function bubbleSort(arrayToSort){ var length = arrayToSort.length; for (var i = 0; i < length-1; i++){ var swapped = false; for (var j = 0; j < length-i-1; j++){ if (arrayToSort\[j\] > arrayToSort\[j+1\]){ swap(arrayToSort, j, j+1); swapped = true; } } if(swapped === false){ i = length; } } } 
``` 
