...
```typescript
function exponentialSearch(arrayToSearch, valueToSearch, length){ if (arrayToSearch[0] == valueToSearch){ return 0; }

var i = 1; while (i < length && arrayToSearch[i] <= valueToSearch){ i = i\*2; } return binarySearch(arrayToSearch, valueToSearch, i/2, Math.min(i, length-1)); } 
``` 
