...
```typescript
function swap(array, left, right) { var temp = array\[left\]; array\[left\] = array\[right\]; array\[right\] = temp; }

function partition(arrayToSort, left, right){ var i = left; var j = right; var pivot = arrayToSort\[Math.floor((left + right) / 2)\]; // Middle

while (i <= j) { while (arrayToSort\[i\] < pivot){ i++; } while (arrayToSort\[j\] > pivot){ j--; } if (i <= j) { swap(arrayToSort, i, j); i++; j--; } }

return i; }

function quickSort(arrayToSort, left, right){ var index = partition(arrayToSort, left, right); if (left < index - 1){ quickSort(arrayToSort, left, index - 1); } if (right > index){ quickSort(arrayToSort, index, right); } } 
``` 
