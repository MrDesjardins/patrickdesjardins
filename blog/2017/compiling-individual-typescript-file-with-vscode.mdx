...
As you can see from the 2 first lines, it needs to have 2 npm packages. ``` 
 npm install gulp --save-savedev npm install gulp-typescript typescript --save-savedev npm install gulp-sourcemaps --save-dev 
``` 
...
To use it, we only need to press "F1" on the single file we want to compile, type "task", select "Run Task" and select "buildsinglefile". ![](images/tasksRunTask.png)

This will generate the map file, as well as the JavaScript file. You can click the following image to see the animation of how it works. ![](images/TypeScriptBuildSingleFile-1024x427.gif)
