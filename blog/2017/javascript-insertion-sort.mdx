...
```typescript
function insertionSort(arrayToSort){ var key; var length = arrayToSort.length; for (var i = 1; i < length; i++) { key = arrayToSort[i]; var j = i-1; while (j >= 0 && arrayToSort[j] > key) { arrayToSort[j+1] = arrayToSort[j]; j = j-1; } arrayToSort[j+1] = key; } } 
``` 
