...
To be able to use Gulp, we need to download gulp. It's convenient since we can get it from npm by executing the install command. \[code\] npm install gulp --save-development //or npm install gulp -g \[/code\]

Create a Gulp's configuration file : **gulpfile.js** at the root, sibling to TypeScript's configuration file, and typings' configuration file. We will need some additional node modules to help us working inside Gulp. You can get the one we will use by typing: \[code\] npm install gulp-typescript --save-development npm install typescript --save-development npm install del --save-development npm install gulp-sourcemaps --save-development \[/code\]

This file is pure JavaScript that use Node's module to execute JavasScript for specific tasks. The task runner is Gulp. The goal is to be able in the console to write: \[code\] gulp clean //or gulp build \[/code\] The Gulp file that we will build is simple for the case of learning how to use Gulp to do:
...
Before creating these 3 Gulp's tasks, we need to instanciate some node's modules that will help us doing the work. ```typescript
var gulp = require('gulp'); var tsc = require('gulp-typescript'); var del = require('del'); var sourcemaps = require('gulp-sourcemaps'); var tsProject = tsc.createProject('tsconfig.json'); 
``` 
...
The next step is to define some constants to help us having a cleaner Gulp code. ```typescript
var paths = { webroot: "./", node\_modules: "./node\_modules/", typescript\_in: "./src/", typescript\_out: "output", typings: "./typings/", typescript\_definitions: "./typings/main/\*\*/\*.ts" }; paths.allTypeScript = paths.typescript\_in + "\*\*/\*.ts"; paths.modulesDestination = paths.webroot + "vendors/"; 
``` 
...
```typescript
gulp.task("clean", function (callback) { var typeScriptGenFiles = \[ paths.typescript\_out + "/\*\*/\*.js", paths.typescript\_out + "/\*\*/\*.js.map" \];
...
```typescript
gulp.task("copy", function () { var modulesToMove = { "jquery": "jquery/dist/jquery\*.{js,map}" }

for (var destinationDir in modulesToMove) { gulp.src(paths.node\_modules + modulesToMove\[destinationDir\]) .pipe(gulp.dest(paths.modulesDestination + destinationDir)); } }); 
``` 
...
gulp.task("build", function () { var sourceTsFiles = \[paths.typescript\_in, paths.typescript\_definitions\]; var compilationResults = tsProject.src() .pipe(sourcemaps.init()) .pipe(tsProject()) compilationResults.dts.pipe(gulp.dest(paths.typescript\_out)); return compilationResults.js .pipe(sourcemaps.write('.')) .pipe(gulp.dest(paths.typescript\_out)); }); 
``` 


So far, we can go in the console and use : gulp clean or gulp copy or gulp build. We saw that we can have a second parameter that is an array of task name, we could have a task that execute these three tasks we just created which will allow us to execute gulp buildall and have the clean, copy and build executed. ```typescript
