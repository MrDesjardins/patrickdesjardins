...
The Python version returns an iterable, which requires using `list` to build a new list. Also, the `map` function needs an anonymous function that takes the transformation first, then the original array. This is the reverse order of JavaScript. While it is more a matter of preference, I found myself having to search more in the documentation or search on the web to find the right function to accomplish the task instead of relying on the autocomplete when using other languages.
